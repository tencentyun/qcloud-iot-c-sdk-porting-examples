...
static void eventPostCheck(void *client)
{
    ...
    if (GetButtonPressedCount() > 0) {
        memset(sg_button_event_msg, 0, 128);
        if (GetLed2State() == eLD2_OFF) {
            HAL_Snprintf(sg_button_event_msg, 128, "led2 off");
        } else {
            HAL_Snprintf(sg_button_event_msg, 128, "led2 on");
        }
 ...
 *
*/
void deal_down_stream_user_logic(void *client, ProductDataDefine   * pIcubeDemo)
{
...
static int _refresh_local_property(void)
{
    ...
    if (hum != sg_ProductData.m_hum) {
        ...
        set_propery_state(&sg_ProductData.m_hum, eCHANGED);
    }

    if (temp != sg_ProductData.m_temperature) {
        sg_ProductData.m_temperature = temp;
        set_propery_state(&sg_ProductData.m_temperature, eCHANGED);
    }
#endif

    if (GetLed2State() != sg_ProductData.m_led_ctrl) {
        sg_ProductData.m_led_ctrl = GetLed2State();
        set_propery_state(&sg_ProductData.m_led_ctrl, eCHANGED);
    }
