...
static int _parse_devinfo(char *jdoc, DeviceInfo *pDevInfo)
{
	...
	if(NULL != clientCert){
		...
		if(QCLOUD_ERR_SUCCESS != _cert_file_save(pDevInfo->dev_cert_file_name, clientCert,	strlen(clientCert))){
	...
	if(NULL != clientKey){
		...
		if(QCLOUD_ERR_SUCCESS != _cert_file_save(pDevInfo->dev_key_file_name, clientKey,	strlen(clientKey))){
