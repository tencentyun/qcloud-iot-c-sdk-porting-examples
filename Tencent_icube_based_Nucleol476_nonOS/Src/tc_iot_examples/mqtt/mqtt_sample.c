...
#define MAX_SIZE_OF_TOPIC_CONTENT 512
...
void event_handler(void *pclient, void *handle_context, MQTTEventMsg *msg)
{
    ...
    (void)sg_sub_packet_id; //eliminate warning
}
...
static int _publish_msg(void *client)
{
    ...
    char topic_content[MAX_SIZE_OF_TOPIC_CONTENT + 1];
    int size;
    ...
    eLd2StateType state = GetLed2State();
...
//  rc = IOT_MQTT_Yield(client, 1000);
    ...
    do {
        ...
        if (GetButtonPressedCount() == 0) {
            Log_d("\n\r=========Press the Blue button triger publish msg to tencent iot_explorer platform=====");
        } else {
